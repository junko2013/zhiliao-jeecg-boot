...
@Service
@Slf4j
public class MucInviteServiceImpl extends BaseServiceImpl<MucInviteMapper, MucInvite> implements IMucInviteService {
    ...
    @Resource
    private IUserSettingService IUserSettingService;
    @Resource
    ...
    public Result<Object> send(Integer userId, Integer mucId, Integer toUserId) {
        try {
            User toUser = IUserService.findById(toUserId);
            Muc muc = IMucService.getById(mucId);
            if (muc == null) {
                return fail("群组不存在");
            }
            MucConfig config = IMucConfigService.findByMuc(mucId);
            if (muc.getMemberCount() >= config.getMaxMemberCount()) {
                return fail("该群成员人数已达上限");
            }
            MucMember inviter = IMucMemberService.findByMucIdOfUser(mucId, userId);
            ...
            MucMember invitee = IMucMemberService.findByMucIdOfUser(mucId, toUserId);
            if (invitee != null) {
    ...
    public Result<Object> passBySelf(Integer userId, Integer fromId, Integer mucId) {
        try {
            ...
            MucMember handler = IMucMemberService.findByMucIdOfUser(mucId, userId);
            ...
            User toUser = IUserService.findById(userId);
            if(handler !=null){
                handler.setStatus(MucMember.Status.normal.getCode());
                handler.setTsJoin(getDate());
            ...
            if (!config.getIsShowMsgBeforeJoin()) {
                //设置可见消息时间为入群时间
                handler.setTsMsgVisible(getDate());
            }
            //新成员不允许发言
            if (!config.getIsAllowTalkAfterJoin()) {
                handler.setTsMute(ToolDateTime.forever());
                handler.setMuteType(User.MuteType.newMember.getCode());
            }
            if(handler.getId()!=null){
                if(!IMucMemberService.updateById(handler)){
                    throw new BusinessException("更新群成员失败");
                }
            }else {
                if (!IMucMemberService.save(handler)) {
                    throw new BusinessException("保存群成员失败");
                }
            }
            muc.setMemberCount(IMucMemberService.getCount(mucId, MucMember.Status.normal.getCode()));
            if(!IMucService.updateById(muc)){
                throw new BusinessException("更新群组失败");
            }
            invite.setStatus(MucInvite.Status.Accept.getCode());
            invite.setTsDeal(getDate());
    ...
    public Result<Object> inviteBatch(Integer inviterId, Integer mucId, String userIds) {
        try{
            ...
            MucMember inviter = IMucMemberService.findByMucIdOfUser(mucId, inviterId);
            if (inviter == null) {
                return fail("你不是该群的成员，无法操作");
            }
            MucConfig mucConfig = IMucConfigService.findByMuc(muc.getId());
            boolean isManager = inviter.getRole() == MucMember.Role.Master.getCode() || inviter.getRole() == MucMember.Role.Manager.getCode();
    ...
    private Result<Object> inviteOne(Integer mucId,Integer userId,MucMember inviter,MucConfig mucConfig) throws Exception {
        ...
        if(!isManager&&mucConfig.getIsJoinVerify()){
            ...
            if(IXMPPService.sendMucMsg(messageBean)){
                return success();
            }
            return fail();
        }else{
            ...
            save(invite);
            IXMPPService.inviteUsers(inviter.getUserId(),mucId,userId.toString());
            //保存群成员
            if(member!=null){
                member.setStatus(MucMember.Status.normal.getCode());
                member.setKicker(0);
                member.setTsQuit(null);
            }else{
            ...
            if (!mucConfig.getIsShowMsgBeforeJoin()) {
                //设置可见消息时间为入群时间
                member.setTsMsgVisible(getDate());
            }
            //新成员不允许发言
            ...
            IMucMemberService.saveOrUpdate(member);
            invite.setInviterMember(inviter);
            invite.setInviteeUser(user);
            //发送新的成员入群
            MessageBean messageBean = new MessageBean();
            ...
            if(!IXMPPService.sendMsgToOne(messageBean2)){
                return fail();
            }
            return success();
        }
